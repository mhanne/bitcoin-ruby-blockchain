...
Bitcoin::NETWORKS[:fake] = Bitcoin::NETWORKS[:testnet3].merge({
  project: :bitcoin,
  no_difficulty: true,
  magic_head: "fake",
...
class FakeBlockchain
  ...
  def generate(num = 50, opts = {})
    ...
    genesis = Bitcoin::Builder.build_block do |blk|
       ...
       blk.tx do |t|
         ...
         t.output {|o| o.value 50*Bitcoin::COIN
          o.script {|s|
        ...
        loop do
          ...
          tx = blk.tx do |t|
            ...
            if rand < many_outputs_prob
              ...
              new_outs << {tx_idx: 0, key: k1, value: v1}
  ...
  def block(depth)
    return  unless File.exist?(block_path(depth))
  ...
  def block_path(depth)
    ...
    "spec/fixtures/fake_chain/#{depth}.blk"
  end
