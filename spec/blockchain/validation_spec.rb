...
[
  ...
  it "should check coinbase output value" do
    ...
    expect { @store.store_block(block3) }.to raise_error(ValidationError)

    Bitcoin::REWARD_DROP = 2
    block4 = create_block(block2.hash, false, [], @key, 50e8)
    expect { @store.store_block(block4) }.to raise_error(ValidationError)
...
describe "transaction rules (#{options[0]} - #{options[1]})" do
  ...
  it "should validate" do
    ...
    expect { validator.validate(raise_errors: true) }.to raise_error(ValidationError)
  end
  ...
  it "should not allow double spend within the same block" do
    ...
    expect { @store.store_block(block) }.to raise_error(ValidationError)
